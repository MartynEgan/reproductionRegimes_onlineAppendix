########################################################
# Data Analysis: GDA (weighted), first factorial plane #
########################################################
...
# Load data
ISSP5 <- readRDS("data/ISSP5_GDA") # 2019 data
ISSP4 <- readRDS("data/ISSP4_GDA") # 2009 data
mod <- readRDS("data/pca_mod") # PCA model
...
el.data <- function(Mcov){
  ...
  theta <- atan(E[[2]][[1,1]]/E[[2]][[2,1]]) * (180/pi) # swap axes to reflect PC1 on y
  out <- list()
  out$eig <- E
  out$D <- D
  out$e <- e
...
c_wt_cos2$y_z <- scale(c_wt_cos2$y, 
                       center = weighted.mean(ISSP5$cos2_1, w = ISSP5$WEIGHT_c),
                       scale = sd(ISSP5$cos2_1))
c_wt_cos2$x_z <- scale(c_wt_cos2$x, 
                       center = weighted.mean(ISSP5$cos2_2, w = ISSP5$WEIGHT_c),
                       scale = sd(ISSP5$cos2_2))
c_wt_cos2 %>%
  arrange(desc(x))
c_wt_cos2 %>%
  arrange(desc(y))
...
cn_wt_cos2 <- m_point(ISSP4, "cos2_2", "cos2_1", index = "c_alphan")
scale(cn_wt_cos2$y, 
      center = weighted.mean(ISSP5$cos2_1, w = ISSP5$WEIGHT_c),
      scale = sd(ISSP5$cos2_1))
...
# Calculate mean cos^2
edu_wt_cos2 <- m_point(ISSP5, "cos2_2", "cos2_1", 
...
# Calculate mean cos^2
isco_wt_cos2 <- m_point(ISSP5, "cos2_2", "cos2_1",
...
sex_wt_cos2 <- m_point(ISSP5, "cos2_2", "cos2_1", index = "SEX", na.rm = TRUE)

# 6. Find mean points for political party
pol_wt_mean <- m_point(ISSP5, index = "PARTY_LR", na.rm = TRUE)
pol_wt_cos2 <- m_point(ISSP5, "cos2_2", "cos2_1", index = "PARTY_LR", na.rm = TRUE)
...
saveRDS(c_wt_gt, "data/c_wt_gt")

# a.ii. China (ISSP4)
cn_wt_gt <- by_gt(ISSP4, index = "c_alphan")
saveRDS(cn_wt_gt, "data/cn_wt_gt")

# b. Using barycentre of weighted cloud as mean
c_wt_cf_gt <- by_gt(ISSP5, index = "c_alphan", 
                    centre = FALSE, 
                    mean = c(weighted.mean(ISSP5$PC2, w = ISSP5$WEIGHT_c),
                             weighted.mean(ISSP5$PC1, w = ISSP5$WEIGHT_c)))
saveRDS(c_wt_cf_gt, "data/c_wt_cf_gt")

## Visualise
## Plot compatibility ellipses
# drop DE, US and VE
c_wt_mean_sig <- c_wt_mean[-c(2,15,16),]
c_wt_gt_sig <- c_wt_gt[-c(2,15,16)]
...
# label adjustments
lab_x <- c(0,0,-0.01,0,0,0,0,0.01,rep(0,8))
lab_y <- c(-0.02,rep(0,15))
# m_distance circle
radius = 0.4
center_x = 0
center_y = 0
...
sig_col <- c_wt_cos2$col[-c(2,15,16)]

# Plot
png(filename = "plots/c_wt_geom.png",
...
text(c(1.05,1.05,-1.05,-1.05,0,0),c(0.03,-0.03,0.03,-0.03,1.15,-0.8),
     ...
     cex = 0.8)
# Add text
text(c_wt_mean$x+lab_x, c_wt_mean$y+lab_y,
...
lines(class_wt_mean$x[2:5], class_wt_mean$y[2:5], 
      lty = 4)
text(class_wt_mean$x+c(0,-0.11,-0.14,0.07,0,0.06), 
     class_wt_mean$y+c(.04,0.03,-.04,-.03,0.04,-0.03),
...
points(socis_wt_mean$x,socis_wt_mean$y,
       pch = 17, col = "grey50")
text(socis_wt_mean$x+c(0,0.02,-0.03,-0.03,-0.03),
     socis_wt_mean$y+c(0.03,0.03,0,0,0),
     label = socis_wt_mean$group, col = "grey50")
lines(socis_wt_mean$x[1:4],socis_wt_mean$y[1:4],
      lty = 4)
legend("topright", legend = c("Social Class","Type of Society",
                              "Weighted Barycenter"),
...
saveRDS(class_wt_gt, "data/class_wt_gt")
...
## Plot
png(filename = "plots/class_wt_geom.png",
    width = 1200, height = 1200)
par(pty = "s", cex = 2)
plot(x = NULL,
     y = NULL,
...
points(class_wt_mean[class_wt_mean$group != "Lower class","x"],
       class_wt_mean[class_wt_mean$group != "Lower class","y"],
       pch = 13, col = "black")
...
saveRDS(isco_wt_gt, "data/isco_wt_gt")
...
## Plot
png(filename = "plots/isco_wt_geom.png",
    width = 1200, height = 1200)
par(pty = "s", cex = 2)
plot(x = NULL,
     y = NULL,
...
saveRDS(edu_wt_gt, "data/edu_wt_gt")

lab_x_5 <- c(0,.18,0,-.15,0,0)
lab_y_5 <- c(-.03,0.05,.03,0.03,-.03,.03)
# m_distance circle
radius = 0.4
center_x = 0
center_y = 0
...
## Plot
png(filename = "plots/edu_wt_geom.png",
    width = 1200, height = 1200)
par(pty = "s", cex = 2)
plot(x = NULL,
     y = NULL,
     main = "Education Mean Points in First Factorial Plane",
     xlab = paste0("PC2, ", round(mod$Vaccounted[[2,2]]*100, 1), "% of variance\n (Capital Structure)"),
     ylab = paste0("PC1, ", round(mod$Vaccounted[[2,1]]*100, 1), "% of variance\n (Capital Volume)"),
     xlim = c(-.8,.8),
     ylim = c(-.8,.8),
...
saveRDS(sex_wt_gt, "data/sex_wt_gt")
...
## Plot
png(filename = "plots/sex_wt_geom.png",
    width = 1200, height = 1200)
par(pty = "s", cex = 2)
plot(x = NULL,
     y = NULL,
     main = "Gender Mean Points in First Factorial Plane",
     xlab = paste0("PC2, ", round(mod$Vaccounted[[2,2]]*100, 1), "% of variance\n (Capital Structure)"),
     ylab = paste0("PC1, ", round(mod$Vaccounted[[2,1]]*100, 1), "% of variance\n (Capital Volume)"),
     xlim = c(-.8,.8),
     ylim = c(-.8,.8),
...
saveRDS(pol_wt_gt, "data/pol_wt_gt")
...
## Plot
png(filename = "plots/pol_wt_geom.png",
    width = 1200, height = 1200)
par(pty = "s", cex = 2)
plot(x = NULL,
     y = NULL,
     main = "Politics Mean Points in First Factorial Plane",
     xlab = paste0("PC2, ", round(mod$Vaccounted[[2,2]]*100, 1), "% of variance\n (Capital Structure)"),
     ylab = paste0("PC1, ", round(mod$Vaccounted[[2,1]]*100, 1), "% of variance\n (Capital Volume)"),
     xlim = c(-.8,.8),
     ylim = c(-.8,.8),

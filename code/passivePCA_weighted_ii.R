#######################################
# Passive PCA, second factorial plane #
#######################################
...
png(filename = "plots/supp_PCA_1_3_wt_vect_CME_LME_Nord.png",
...
arrows(0,0,mean(cme_5$x-cme_4$x),mean(cme_5$y-cme_4$y),
       ...
       lwd = 2,
       col = "black")
text(mean(cme_5$x-cme_4$x),mean(cme_5$y-cme_4$y),
     pos = 3,
     label = "CME",
     cex = 0.7)
# CMEs
...
ev_gt <- gt.test(base)
# M distance: 0.481 (notable)
# P value: 15172/65536 = 0.232
# Kappa: 0.74 (95% confidence region between 0.735 and 0.745)

base <- ISSP4[,c("PC3","PC1")]
Mcov.KK <- cov.wt(base, wt = ISSP4$WEIGHT_c, method = "ML")$cov
ev_full_gt <- gt.test(base, centre = FALSE, mean = c(
  weighted.mean(ISSP5$PC3, w = ISSP5$WEIGHT_c),
  weighted.mean(ISSP5$PC1, w = ISSP5$WEIGHT_c)
))

## Plot effect-vectors with compatibility ellipse (geometric typicality test)
# label adjustment
pos <- c(1,4,2,4,1,1,3,1,3,3,3,2,4,3,3,2) 
png(filename = "results/c_alphan_1_3_wt_vectors.png",
    width = 1400, height = 1400)
par(pty = "s",
    cex = 2.5)
plot(main = NULL,#"Effect-Vectors by Country, 2009-2019 \n (Supplementary PCA, PCs 1 and 3)",
     NULL,
...
for (i in 1:length(c_alph)) {
  base <- ISSP5[ISSP5$c_alphan %in% c_alph[i],c("PC3","PC1")]
  wt <- ISSP5[ISSP5$c_alphan %in% c_alph[i],"WEIGHT_c"]
  group <- ISSP4[ISSP4$c_alphan %in% c_alph[i],c("PC3","PC1")]
...
# label adjustments
pos_4 <- c(3,2,1,2,4,4,1,2,2,1,1,3,1,1,1,4)
pos_5 <- c(2,4,3,4,1,3,3,4,4,4,3,2,4,3,3,2)
...
# Plot
png(filename = "results/c_alphan_1_3_wt_supp_PCA.png",
...
plot(c_5_wt_mean$x, c_5_wt_mean$y,
     ...
     xlab = paste0("PC3, ", round(mod$Vaccounted[[2,3]]*100, 1), "% of variance\n (Capital Structure/Symbolic System)"),
     ylab = paste0("PC1, ", round(mod$Vaccounted[[2,1]]*100, 1), "% of variance\n (Capital Volume)"),
     main = NULL, #"Country Transformations in PCs 1 and 3 \n (Supplementary PCA)",
     xlim = c(-0.8,0.8),
     ylim = c(-0.8,0.8),
     asp = 1)

## Edited version of biplot function from psych package (W.R. Revelle) to change
## colour of cloud of points gray
...
"biplot.edit" <-
  function(x, labels=NULL,cex=c(.75,1),main="Biplot from fa",hist.col="cyan",xlim.s=c(-3,3),ylim.s=c(-3,3),xlim.f=c(-1,1),ylim.f=c(-1,1),maxpoints=100,adjust=1.2,col,pos, arrow.len = 0.1,pch=16,choose=NULL,cuts=1,cutl=.0,group=NULL,smoother = FALSE,vars=TRUE,...) {
    ...
    if (n.dims == 2) { #just do a one panel graph
      ...
      plotone(x$scores,x$loading,labels=labels,main=main,xlim.s=xlim.s,ylim.s=ylim.s,xlim.f=xlim.f,ylim.f=ylim.f,maxpoints=maxpoints,adjust=adjust,col=col,cex=cex,pos=pos, arrow.len = arrow.len,pch=pch,choose=choose,cuts=cuts,cutl=cutl,group=group,ch.col=ch.col,smoother=smoother,vars=vars,... )
...
plotone <- function( scores,loadings,labels=NULL,cex=c(.75,1),main=main,hist.col="cyan",xlim.s=c(-3,3),ylim.s=c(-3,3),xlim.f=c(-1,1),ylim.f=c(-1,1),maxpoints=100,adjust=1.2,col,pos, arrow.len = 0.1,pch=16,choose=NULL,cuts=1,cutl=.0,group=NULL,ch.col=c("black","blue"),smoother=FALSE, vars=TRUE,...)  { 
  ...
  switch(choice,   #we plot the scores here for scores > abs(cut) on x and y
         ...
         "smoother" = {smoothScatter(scores, nrpoints=0, colramp = colorRampPalette(c("white", "white", "gray40"), space = "Lab"), bandwidth = c(0.15), xlim = xlim.s, ylim = ylim.s)
           radius = 0.4
           center_x = 0
           center_y = 0
           theta = seq(0, 2 * pi, length = 200)
           lines(x = radius * cos(theta) + center_x, y = radius * sin(theta) + center_y, col = "grey80")
         }
  )
